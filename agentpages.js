...
function sanitizeHref(url) {
  ...
  const ICONS = {
    ...
    alert: '<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" fill="none" stroke="currentColor" stroke-width="2" /><line x1="12" y1="9" x2="12" y2="13" stroke="currentColor" stroke-width="2" /><line x1="12" y1="17" x2="12.01" y2="17" stroke="currentColor" stroke-width="2" />',
...
function cssEscapeIdent(ident) {
  ...
  // Minimal fallback (good enough for internal ids). Escapes non-identifier chars and leading digits.
  return s
    .replace(/(^-?\d)/, '\\3$1 ')
  ...
  function generateCSS(tokens, themeColors, containerId) {
    const c = `#${cssEscapeIdent(containerId)}`;
...
--ap-space-0: 0px;
--ap-space-1: ${tokens.space1};
--ap-space-2: ${tokens.space2};
--ap-space-3: ${tokens.space3};
--ap-space-4: ${tokens.space4};
--ap-space-5: ${tokens.space5};
--ap-space-6: ${tokens.space6};
--ap-space-8: ${tokens.space8};
--ap-space-10: ${tokens.space10};
--ap-space-12: ${tokens.space12};
...
`;
    ...
    if (data.cta && data.cta.text) {
      const btn = el(
        'a',
        {
          ...
          href: data.cta.url ? sanitizeHref(data.cta.url) : '#',
  ...
  RENDERERS.stack = function (data, renderSection) {
    ...
    const rawGap = Number(data.gap);
const gap = Number.isFinite(rawGap) ? Math.min(Math.max(rawGap, 0), 12) : 4;
const allowed = [0, 1, 2, 3, 4, 5, 6, 8, 10, 12];
const snapped = allowed.reduce((best, cur) =>
  Math.abs(cur - gap) < Math.abs(best - gap) ? cur : best, allowed[0]
);

const stack = el('div', { className: 'ap-stack' });
stack.style.gap = `var(--ap-space-${snapped})`;
    ...
    if (data.primary && data.primary.text) {
      btnRow.appendChild(
        el(
          ...
          {
            className: 'ap-btn ap-btn--primary',
            href: data.primary.url ? sanitizeHref(data.primary.url) : '#',
      ...
      btnRow.appendChild(
        el(
          'a',
          {
            ...
            href: data.secondary.url ? sanitizeHref(data.secondary.url) : '#',
    ...
    function validateField(field, value) {
      ...
      if (value !== null && value !== undefined) {
        ...
        if (field.pattern && typeof field.pattern === 'string') {
          const pat = String(field.pattern);
          // Defensive: avoid pathological regex patterns (ReDoS) and very long inputs.
          const MAX_PATTERN_LEN = 200;
          const MAX_TEST_LEN = 256;

          const isProbablySafe = (p) => {
            if (!p || p.length > MAX_PATTERN_LEN) return false;
            // Reject backreferences and lookbehinds (complex + can be expensive).
            if (/\\\d/.test(p)) return false;
            if (/\(\?<([=!])/.test(p)) return false; // lookbehind
            // Heuristic: reject nested quantifiers like (a+)+, (.*)+, (.+)*, (a{1,3})+
            if (/\([^)]*[+*{][^)]*\)\s*[+*{]/.test(p)) return false;
            // Heuristic: reject dot-star/dot-plus quantified groups repeated.
            if (/\(\?:?\s*\.[*+][^)]*\)\s*[+*{]/.test(p)) return false;
            return true;
          };

          if (isProbablySafe(pat)) {
            try {
              const re = new RegExp(pat);
              const sample = s && s.length > MAX_TEST_LEN ? s.slice(0, MAX_TEST_LEN) : s;
              if (sample && !re.test(sample)) errors.push(field.errorPattern || label + ' is invalid.');
            } catch (e) {
  ...
  class AgentPages {
    ...
    constructor(options) {
      ...
      this._userOnAction = typeof options.onAction === 'function' ? options.onAction : null;
      this.onAction = this._dispatchAction.bind(this);
    ...
    injectStyles() {
      ...
      if (existing) {
  if (this.cspNonce) existing.setAttribute('nonce', this.cspNonce);
  existing.textContent = generateCSS(this.tokens, this.colors, this.containerId);
  return;
}
...
rootNode.appendChild(style);
    ...
    addSection(sectionData) {
      ...
      this._sectionNodes.push(node);
      this._sections.push(normalized[0]);
      this.sectionCount = this._sectionNodes.length;
      node.setAttribute('data-ap-index', String(this.sectionCount - 1));
      this._emit('section:added', { index: this.sectionCount - 1, section: normalized[0] });
    }

    render(pageData) {
      this.clear();
      this._emit('render:start', { page: null });
      ...
      this._emit('render:complete', { sections: this._sections.length });
    }

    async renderStream(stream) {
      this.clear();
      this._emit('stream:start', { mode: 'json' });
      ...
      this._emit('stream:complete', { mode: 'json', sections: this._sections.length });
    }

    async renderStreamNDJSON(stream) {
      this.clear();
      this._emit('stream:start', { mode: 'ndjson' });
      ...
      await this._consumeStream(stream, (chunk) => parser.feed(chunk));
      parser.flush();
      this._emit('stream:complete', { mode: 'ndjson', sections: this._sections.length });
    }
...
on(eventName, handler) {
  ...
  if (!this._events[key]) this._events[key] = new Set();
  this._events[key].add(handler);
  return () => this.off(key, handler);
}

off(eventName, handler) {
  const key = String(eventName || '');
  const set = this._events[key];
  if (!set) return;
  set.delete(handler);
  if (set.size === 0) delete this._events[key];
}
...
_dispatchAction(actionName, event, payload) {
  const name = sanitizeActionName(actionName);
  const allowed = this._shouldAllowAction(name);
  const meta = { name, allowed, payload: payload || null, timestamp: Date.now() };

  this._emit('action', meta);

  if (!allowed) {
    this._emit('action:blocked', meta);
    return;
  }

  if (this._userOnAction) {
    try { this._userOnAction(name, event, payload); } catch (e) { /* swallow */ }
  }
}

_reindex() {
  for (let i = 0; i < this._sectionNodes.length; i++) {
    const n = this._sectionNodes[i];
    if (n && n.setAttribute) n.setAttribute('data-ap-index', String(i));
  }
}

updateSection(index, sectionData) {
  const i = Number(index);
  if (!Number.isFinite(i) || i < 0 || i >= this._sectionNodes.length) return false;

  const normalized = this._normalizeSections([sectionData], 0);
  if (!normalized.length) return false;

  const nextNode = this.renderSection(normalized[0]);
  if (!nextNode) return false;

  // Wire click actions for nodes that use [data-action] (same behavior as addSection()).
  if (nextNode.querySelectorAll) {
    nextNode.querySelectorAll('[data-action]').forEach((el) => {
      el.addEventListener('click', (e) => {
        const actionName = sanitizeActionName(el.getAttribute('data-action'));
        if (!this._shouldAllowAction(actionName)) {
          this._emit('action:blocked', { name: actionName, allowed: false, payload: null, timestamp: Date.now() });
          return;
        }
        this.onAction(actionName, e, { source: 'click' });
      });
  ...
  const prevNode = this._sectionNodes[i];
  if (prevNode && prevNode.parentNode) {
    prevNode.parentNode.replaceChild(nextNode, prevNode);
  }

  this._sectionNodes[i] = nextNode;
  this._sections[i] = normalized[0];
  this._reindex();
  this._emit('section:updated', { index: i, section: normalized[0] });
  return true;
}
...
removeSection(index) {
  ...
  const node = this._sectionNodes[i];
  if (node && node.parentNode) node.parentNode.removeChild(node);

  this._sectionNodes.splice(i, 1);
  this._sections.splice(i, 1);
  this.sectionCount = this._sectionNodes.length;
  this._reindex();
  this._emit('section:removed', { index: i });
  return true;
}
...
toJSON() {
  return deepClone({
    ...
    style: { background: this.background || null },
    colors: this.customColors ? deepClone(this.customColors) : {},
    sections: deepClone(this._sections),
  });
}
